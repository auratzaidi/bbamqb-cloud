...
def whatsapp_receipt_url(r):
    ...
    if not phone:
        return "#"
    message = f"""Assalaamu Alaikum.

Your Qurbani/Aqeeqa entry has been successfully recorded.

Receipt No: {r.get('receipt_no','')}
Name: {r.get('client_name','')}
Product: {r.get('product','')}
Shares/Quantity: {r.get('quantity','')}
Amount Recorded: {rupees(r.get('amount_due',0))}
Date/Time: {r.get('created_at','')}

Bazme Barkate Aley Mustafa Trust
Mumbai
...
.small{font-size:12px;color:#6d756f}.danger-link{color:#a83a32}
...
class App(BaseHTTPRequestHandler):
    ...
    def client_row(self,r, trash=False):
        ...
        whatsapp = f" · <a target='_blank' href='{wa}'>WhatsApp</a>" if wa != "#" else ""
        actions = f"<a href='/receipt/{r['id']}'>Receipt</a>{whatsapp} · <a href='/client/edit/{r['id']}'>Edit</a> · <a class='danger-link' href='/client/delete/{r['id']}' onclick=\"return confirm('Move this entry to trash?')\">🗑</a>"
        if trash:
    ...
    def save_client(self,u):
        ...
        if cid:
            ...
            conn.commit(); cur.close(); conn.close(); self.sendx(*redirect(f"/receipt/{new_id}")); return
    ...
    def receipt(self,u,cid):
        ...
        <tr><td>Operator</td><td>{escape(r['created_by'] or '')}</td></tr></table>
        <a class='btn' target='_blank' href='{whatsapp_receipt_url(r)}'>Send WhatsApp Receipt</a>
        <a class='btn secondary' href='/search'>Back to Search</a>
        </div>"""
    ...
    def export_csv(self):
        # SMS export: telephone numbers only.
        # Active entries only. Empty numbers are ignored.
        # DISTINCT prevents duplicate phone numbers from repeated entries.
        conn=get_conn(); cur=conn.cursor()
        cur.execute("""
        ...
        self.sendx(
            200,
            {
                ...
                "Content-Disposition":"attachment; filename=bbamqb_sms_numbers.csv"
            },
            out.getvalue()
        )
