...
class App(BaseHTTPRequestHandler):
    ...
    def client_form(self,u,client_id=None,multi=False):
        ...
        body=f"<div class='top'><div><h1>{title}</h1></div></div><div class='card'><form method='post' action='/client/save'><input type='hidden' name='id' value='{val('id')}'><div class='row'><label>Client Name<input name='client_name' required value='{val('client_name')}'></label><label>Mobile<input name='mobile' value='{val('mobile')}'></label><label>City<input name='city' value='{val('city')}'></label><label>Product<select name='product'>{product_opts}</select></label><label>Quantity<input type='number' min='1' name='quantity' value='{val('quantity',7 if multi else 1)}'></label><label>Amount Paid<input type='number' min='0' name='amount_paid' value='{val('amount_paid','')}'></label><label>Payment Status<select name='payment_status'>{status_opts}</select></label><label>Address<input name='address' value='{val('address')}'></label></div><label>Names / Share Holders<textarea name='names'>{escape((r['names'] if r else names_default) or '')}</textarea></label><label>Notes<textarea name='notes'>{val('notes')}</textarea></label><button class='btn'>Save Entry</button></form></div>"
