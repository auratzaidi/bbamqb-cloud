...
def receipt_no():
    # Short mobile-friendly receipt number. Example: BQ-153045-A1B2
    return "BQ-" + datetime.now().strftime("%H%M%S") + "-" + secrets.token_hex(2).upper()
...
.receipt-short{font-family:monospace;font-size:12px;word-break:break-all;max-width:120px;display:inline-block;}
@media(max-width:850px){.receipt-short{font-size:11px;max-width:95px;}}
...
def send_sms(phone, client_name="", receipt_no_value=""):
    """Send automatic SMS through Mobilogi gateway. Does not block saving if failed."""
    if not phone:
        return False
    try:
        ...
        message = "Your Name has been entered in our Database BBAMQB - NUVANTECH"
        url = (
            ...
            "&templateid=1707171569763280386"
        )
        resp = requests.get(url, timeout=10)
        print("SMS SENT:", clean_phone, resp.status_code)
        return True
...
class App(BaseHTTPRequestHandler):
    ...
    def client_row(self,r, trash=False):
        ...
        return f"<tr><td><span class='receipt-short'>{escape(r['receipt_no'])}</span></td><td><b>{escape(r['client_name'])}</b>{extra_names}</td><td>{escape(r['mobile'] or '')}</td><td>{escape(r['product'])}<br><small>{escape(r.get('janwar_no') or '')}</small></td><td>{r['quantity']}</td><td>{rupees(r['amount_due'])}</td><td><span class='small'>{escape(r['created_at'])}</span></td><td>{escape(r['created_by'] or '')}</td><td>{actions}</td></tr>"
    ...
    def client_form(self,u,client_id=None,multi=False):
        ...
        if multi and not r:
            ...
            document.addEventListener('input', updateCombinedMobiles);
            document.addEventListener('change', updateCombinedMobiles);
            </script>"""
    ...
    def save_client(self,u):
        ...
        if d.get("multi_mode", [""])[0] == "1":
            ...
            for i in range(1,8):
                ...
                new_id_saved = cur.fetchone()[0]
                new_ids.append(str(new_id_saved))
                send_sms(phone, name, "")
        ...
        if cid:
            ...
            conn.commit(); cur.close(); conn.close(); send_sms(d.get("mobile", [""])[0].strip(), d.get("client_name", [""])[0].strip(), rec_no)
            self.sendx(*redirect("/"))
            return
    ...
    def receipt(self,u,cid):
        ...
        <tr><td>Receipt</td><td><span class='receipt-short'>{escape(r['receipt_no'])}</span></td></tr><tr><td>Date/Time</td><td>{escape(r['created_at'])}</td></tr>
    ...
    def export_csv(self):
        # ADMIN ONLY: Janwar-wise phone number CSV.
        # Output:
        # Janwar Number,Mobile Numbers
        # Janwar 1,"9821244784,9769800684,"
        u = self.user()
        ...
        grouped = {}
        for r in rows:
            janwar = str(r.get("janwar_no") or "").strip()
            if not janwar:
                janwar = "Unassigned"
        ...
        for janwar in sorted(grouped.keys()):
            sms_line = ",".join(grouped[janwar]) + ","
