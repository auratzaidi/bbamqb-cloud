...
def page(title, body, user=None):
    ...
    if user:
        if user.get("role") == "ROLE_OPERATOR":
            ...
            menu = "<a href='/'>Dashboard</a><a href='/clients'>Clients</a><a href='/client/new'>Single Entry</a><a href='/client/multi'>7 Name Entry</a><a href='/search'>Search</a><a href='/operator-summary'>Operator Summary</a><a href='/trash'>Deleted Entries</a><a href='/report'>Report</a><a href='/export.csv'>SMS Export</a><a href='/users'>Users</a><a href='/logout'>Logout</a>"
...
class App(BaseHTTPRequestHandler):
    ...
    def client_row(self,r, trash=False):
        ...
        return f"<tr><td>{escape(r['receipt_no'])}</td><td><b>{escape(r['client_name'])}</b>{extra_names}<br><small>{escape(r['city'] or '')}</small></td><td>{escape(r['mobile'] or '')}</td><td>{escape(r['product'])}</td><td>{r['quantity']}</td><td>{rupees(r['amount_due'])}</td><td><span class='small'>{escape(r['created_at'])}</span></td><td>{escape(r['created_by'] or '')}</td><td>{actions}</td></tr>"
    ...
    def clients(self,u):
        ...
        <div class='card no-print'><form method='get' action='/clients'><div class='row'><label>Search Name / Phone / Receipt<input name='q' value='{escape(search)}'></label><label>&nbsp;<button class='btn' type='submit'>Search</button></label></div></form></div>
    ...
    def client_form(self,u,client_id=None,multi=False):
        ...
        if multi and not r:
            ...
            {rows}<button class='btn' type='submit'>Save 7 Entries</button></form></div>"""
        ...
        <label>Notes<textarea name='notes'>{val('notes')}</textarea></label><button class='btn' type='submit'>Save Entry</button></form></div>"""
    ...
    def save_client(self,u):
        ...
        if d.get("multi_mode", [""])[0] == "1":
            ...
            for i in range(1,8):
                ...
                cur.execute(
                    "INSERT INTO clients(receipt_no,client_name,mobile,city,address,product,quantity,amount_due,amount_paid,payment_status,names,notes,created_by,created_at,updated_at) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",
        ...
        if cid:
            ...
            rec_no = receipt_no()
            cur.execute("INSERT INTO clients(receipt_no,client_name,mobile,city,address,product,quantity,amount_due,amount_paid,payment_status,names,notes,created_by,created_at,updated_at) VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s) RETURNING id",
                (rec_no,)+vals[:11]+(u["username"],now(),now()))
    ...
    def export_csv(self):
        # ADMIN ONLY: Fast SMS number export, displayed as 7-column plain text/table.
        u = self.user()
        ...
        numbers = [str(r["mobile"]).strip() for r in rows if str(r["mobile"]).strip()]
        chunks = [numbers[i:i+7] for i in range(0, len(numbers), 7)]

        table_rows = ""
        text_lines = []
        for chunk in chunks:
            padded = chunk + [""] * (7 - len(chunk))
            table_rows += "<tr>" + "".join(f"<td>{escape(n)}</td>" for n in padded) + "</tr>"
        ...
        <div class='top'>
          <div>
            <h1>SMS Number Export</h1>
            <p>Admin-only export. Numbers are stacked in rows of 7 for bulk SMS copy-paste.</p>
          </div>
        </div>

        <div class='card no-print'>
          <p><b>Total unique numbers:</b> {len(numbers)}</p>
          <button class='btn' onclick='copyNumbers()'>Copy All Numbers</button>
          <a class='btn secondary' href='/'>Dashboard</a>
        </div>

        <div class='card'>
          <textarea id='smsNumbers' style='width:100%;min-height:220px;font-family:monospace;' readonly>{escape(plain_text)}</textarea>
        </div>
        ...
        function copyNumbers(){{
            var box = document.getElementById('smsNumbers');
            box.select();
        ...
        """
        self.sendx(200, {"Content-Type":"text/html; charset=utf-8"}, page("SMS Number Export",body,u))
    ...
    def user_form(self,u):
        if u["role"]!="ROLE_ADMIN": return self.sendx(*redirect("/"))
        body=f"<div class='top'><div><h1>New User</h1></div></div><div class='card'><form method='post' action='/user/save'><div class='row'><label>Username<input name='username' required></label><label>Password<input name='password' required></label><label>Mobile<input name='mobile'></label><label>Role<select name='role'>{''.join(f'<option>{r}</option>' for r in ROLES)}</select></label></div><button class='btn' type='submit'>Save User</button></form></div>"
