...
label{font-weight:800;font-size:13px;color:#314039;display:block;margin-bottom:12px}input,select,textarea{width:100%;padding:12px;border:1px solid #e0d5c4;border-radius:12px;font:inherit;margin-top:6px}textarea{min-height:90px}.row{display:grid;grid-template-columns:1fr 1fr;gap:14px}
...
class App(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        self.sendx(*redirect("/"))
    def login(self,msg=""):
        body=f"<div class='login'><h1>BBAMQB Login</h1>{f'<div class=notice>{escape(msg)}</div>' if msg else ''}<form method='post' action='/login'><label>Username<input name='username' value='pyra'></label><label>Password<input type='password' name='password' value='admin123'></label><button class='btn' style='width:100%'>Login</button></form></div>"
    ...
    def client_form(self,u,client_id=None,multi=False):
        ...
        body=f"<div class='top'><div><h1>{title}</h1></div></div><div class='card'><form method='post' action='/client/save'><input type='hidden' name='id' value='{val('id')}'><div class='row'><label>Client Name<input name='client_name' required value='{val('client_name')}'></label><label>Mobile<input name='mobile' value='{val('mobile')}'></label><label>City<input name='city' value='{val('city')}'></label><label>Product<select name='product'>{product_opts}</select></label><label>Quantity<input type='number' min='1' name='quantity' value='{val('quantity',7 if multi else 1)}'></label><label>Amount Paid<input type='number' min='0' name='amount_paid' value='{val('amount_paid',0)}'></label><label>Payment Status<select name='payment_status'>{status_opts}</select></label><label>Address<input name='address' value='{val('address')}'></label></div><label>Names / Share Holders<textarea name='names'>{escape((r['names'] if r else names_default) or '')}</textarea></label><label>Notes<textarea name='notes'>{val('notes')}</textarea></label><button class='btn'>Save Entry</button></form></div>"
        self.sendx(200, {"Content-Type":"text/html; charset=utf-8"}, page(title,body,u))
    def save_client(self,u):
        d=self.form(); cid=d.get("id",[""])[0].strip(); product=d.get("product",["Hissa"])[0]; qty=max(1,int(d.get("quantity",["1"])[0] or 1)); paid=int(d.get("amount_paid",["0"])[0] or 0); due=PRODUCTS.get(product,0)*qty
