...
class App(BaseHTTPRequestHandler):
    ...
    def user_form(self,u,uid=None):
        ...
        def val(k,d=""):
            return escape(str(r[k] if r and r.get(k) is not None else d))

        body=f"""
        <div class='top'>
          <div>
            <h1>{'Edit User' if r else 'Add User'}</h1>
          </div>
        </div>
          ...
          <form method='post' action='/user/save'>
            ...
            <div class='row'>
              ...
              <label>
                Role
                <select name='role'>
                  <option value='ROLE_OPERATOR' {'selected' if r and r.get('role')=='ROLE_OPERATOR' else ''}>Operator</option>
                  <option value='ROLE_ADMIN' {'selected' if r and r.get('role')=='ROLE_ADMIN' else ''}>Admin</option>
                </select>
              </label>
            </div>
        ...
        """
        self.sendx(200, {"Content-Type":"text/html; charset=utf-8"}, page("User Form",body,u))
        ...
        if uid:
            if password.strip():
                cur.execute(
                    ...
                    (username,password_hash(password),role,uid)
                )
            else:
                cur.execute(
            ...
            cur.execute(
                ...
                (username,password_hash(password),role,now())
            )
