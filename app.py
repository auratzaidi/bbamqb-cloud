...
class App(BaseHTTPRequestHandler):
    ...
    def export_csv(self):
        # ADMIN ONLY: SMS CSV export exactly matching provided sample format.
        # Output:
        ...
        numbers = []
        for r in rows:
            raw = str(r.get("mobile") or "").strip()
            if not raw:
                continue
            digits = "".join(ch for ch in raw if ch.isdigit())
            if not digits:
                continue
            # IMPORTANT: preserve duplicates exactly like the provided CSV sample.
