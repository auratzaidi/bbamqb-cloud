...
class App(BaseHTTPRequestHandler):
    ...
    def client_row(self,r, trash=False):
        ...
        return f"<tr><td>{escape(r['receipt_no'])}</td><td><b>{escape(r['client_name'])}</b>{extra_names}</td><td>{escape(r['mobile'] or '')}</td><td>{escape(r['product'])}</td><td>{r['quantity']}</td><td>{rupees(r['amount_due'])}</td><td><span class='small'>{escape(r['created_at'])}</span></td><td>{escape(r['created_by'] or '')}</td><td>{actions}</td></tr>"
    ...
    def save_client(self,u):
        ...
        vals=(d.get("client_name",[""])[0].strip(),d.get("mobile",[""])[0].strip(),"","",product,qty,due,due,"Paid",d.get("client_name",[""])[0].strip(),d.get("notes",[""])[0].strip(),now())
        conn=get_conn(); cur=conn.cursor()
        if cid:
