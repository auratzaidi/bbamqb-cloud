...
def page(title, body, user=None):
    ...
    if user:
        if user.get("role") == "ROLE_OPERATOR":
            ...
            menu = "<a href='/'>Dashboard</a><a href='/clients'>Clients</a><a href='/client/new'>Single Entry</a><a href='/client/multi'>7 Name Entry</a><a href='/report'>Reports</a><a href='/export.csv'>CSV Export</a><a href='/users'>Users</a><a href='/logout'>Logout</a>"
...
class App(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # Operator can only use dashboard, single entry, 7-name entry, and logout.
    ...
    def dashboard(self,u):
        rows,due,paid=self.stats()
        recent="".join(self.client_row(r) for r in rows[:8]) or "<tr><td colspan=9>No entries yet.</td></tr>"

        # Operator dashboard: only entries and collected amount.
        if u.get("role") == "ROLE_OPERATOR":
            operator_rows = [r for r in rows if r.get("created_by") == u.get("username")]
            operator_entries = len(operator_rows)
            # Calculation: collected value based on entries/shares.
            # Hissa/Aqeeqa Girl = 1600 each; Aqeeqa Boy = 3200.
            operator_collected = sum(int(r.get("amount_due") or 0) for r in operator_rows)
            body=f"""
            <div class='top'><div><h1>Dashboard</h1><p>Operator entry summary</p></div></div>
            <div class='grid' style='grid-template-columns:repeat(2,minmax(0,1fr));'>
              <div class='stat'><b>{operator_entries}</b><span>Total Entries</span></div>
              <div class='stat'><b>{rupees(operator_collected)}</b><span>Amount Collected</span></div>
            </div>
            <div class='card'>
              <a class='btn' href='/client/new'>Single Entry</a>
              <a class='btn gold' href='/client/multi'>7 Name Entry</a>
            </div>
            """
            self.sendx(200, {"Content-Type":"text/html; charset=utf-8"}, page("Dashboard",body,u))
            return

        # Admin dashboard: full operational overview.
        balance=max(0,due-paid)
        body=f"<div class='top'><div><h1>Dashboard</h1><p>Cloud PostgreSQL Operations Suite</p></div></div><div class='grid'><div class='stat'><b>{len(rows)}</b><span>Entries</span></div><div class='stat'><b>{sum(int(r['quantity'] or 0) for r in rows)}</b><span>Shares</span></div><div class='stat'><b>{rupees(due)}</b><span>Total Due</span></div><div class='stat'><b>{rupees(paid)}</b><span>Collected</span></div></div><div class='card'><a class='btn' href='/client/new'>Single Entry</a><a class='btn gold' href='/client/multi'>7 Name Entry</a><a class='btn secondary' href='/report'>Report</a></div><div class='card'><h2>Recent Entries</h2><table>{self.client_head()}{recent}</table></div>"
        self.sendx(200, {"Content-Type":"text/html; charset=utf-8"}, page("Dashboard",body,u))

    def clients(self,u):
        rows=self.all_clients(); body=f"<div class='top'><div><h1>Clients</h1><p>Manage records</p></div><a class='btn' href='/client/new'>Single Entry</a></div><div class='card'><table>{self.client_head()}{''.join(self.client_row(r) for r in rows) or '<tr><td colspan=9>No data</td></tr>'}</table></div>"
