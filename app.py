...
def page(title, body, user=None):
    ...
    if user:
        if user.get("role") == "ROLE_OPERATOR":
            ...
            menu = "<a href='/'>Dashboard</a><a href='/clients'>Clients</a><a href='/client/new'>Single Entry</a><a href='/client/multi'>7 Name Entry</a><a href='/search'>Search</a><a href='/operator-summary'>Operator Summary</a><a href='/trash'>Deleted Entries</a><a href='/report'>Report</a><a href='/export.csv'>SMS Export</a><a href='/janwar.csv'>Janwar CSV</a><a href='/users'>Users</a><a href='/logout'>Logout</a>"
...
class App(BaseHTTPRequestHandler):
    ...
    def client_form(self,u,client_id=None,multi=False):
        ...
        if multi and not r:
            ...
            {rows}
            <div class='notice' id='capacity_notice'>If Aqeeqa Boy is selected, the 7th row will be disabled because Aqeeqa Boy uses 2 hissas.</div>
            ...
            function updateSevenCapacity(){{
                ...
                for (var i=1;i<=7;i++){{
                    ...
                    if (!p) continue;
                    var active = !(i === 7 && boySelected);
                    if (active) {{
                        used += (p.value === 'Aqeeqa Boy') ? 2 : 1;
                        if (p.value === 'Aqeeqa Boy') boySelected = true;
                    }}
                }}

                var disable7 = false;
                for (var j=1;j<=6;j++){{
                    var pj = document.getElementById('product_'+j);
                    if (pj && pj.value === 'Aqeeqa Boy') disable7 = true;
                }}

                var row7 = document.getElementById('row_7');
                ['name_7','phone_7','product_7','note_7'].forEach(function(id){{
                    var el = document.getElementById(id);
                    if (el) {{
                        el.disabled = disable7;
                        if (disable7 && id !== 'product_7') el.value = '';
                    }}
                }});
                if (row7) {{
                    row7.style.opacity = disable7 ? '.45' : '1';
                    row7.style.pointerEvents = disable7 ? 'none' : 'auto';
                }}

                var finalUsed = 0;
                for (var k=1;k<=7;k++){{
                    var pk = document.getElementById('product_'+k);
                    var nk = document.getElementById('name_'+k);
                    var phk = document.getElementById('phone_'+k);
                    if (!pk || pk.disabled) continue;
                    if ((nk && nk.value.trim()) || (phk && phk.value.trim())) {{
                        finalUsed += (pk.value === 'Aqeeqa Boy') ? 2 : 1;
                    }}
                }}
                var status = document.getElementById('capacity_status');
                if (status) status.value = finalUsed + ' / 7 hissas used';
            }}
    ...
    def save_client(self,u):
        ...
        if d.get("multi_mode", [""])[0] == "1":
            ...
            boy_seen = False
            for i in range(1,8):
                # If Aqeeqa Boy already selected in rows 1-6, ignore row 7 server-side too.
                if i == 7 and boy_seen:
                    continue
